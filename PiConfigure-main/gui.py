import sys, os
from PyQt5.QtWidgets import QApplication, QMainWindow,QPushButton, QInputDialog, QCheckBox, QComboBox, QLineEdit, QLabel, QVBoxLayout, QRadioButton ,QWidget, QFileDialog, QHBoxLayout
...
class RpiCreator(QMainWindow):
    ...
    def main_window(self):
        ...
        self.program_field = QVBoxLayout()
        self.python2_checkBox = QCheckBox("Program napisany w python2", self)
        self.python2_checkBox.stateChanged.connect(self.toggle2ComboBox) 
        self.python3_checkBox = QCheckBox("Program napisany w python3", self)
        self.python3_checkBox.stateChanged.connect(self.toggle3ComboBox) 
        self.python2_comboBox = QComboBox(self)
        self.python2_comboBox.setEnabled(False) 
        self.python3_comboBox = QComboBox(self)
        self.python3_comboBox.setEnabled(False) 
        self.program_field.addWidget(self.python2_checkBox)
        self.program_field.addWidget(self.python2_comboBox) 
        self.program_field.addWidget(self.python3_checkBox)
        self.program_field.addWidget(self.python3_comboBox)          
    ...
    def toggle2ComboBox(self, state):
        if state == Qt.Checked:
            options = self.system.get_programs('python2')
            
            for option in options:
                self.python2_comboBox.addItem(option)
            self.python2_comboBox.setEnabled(True)
        else:
            self.python2_comboBox.setEnabled(False)
    
    def toggle3ComboBox(self, state):
        if state == Qt.Checked:
            options = self.system.get_programs('python3')
            for option in options:
                self.python3_comboBox.addItem(option)
            self.python3_comboBox.setEnabled(True)
        else:
            self.python3_comboBox.setEnabled(False)
    ...
    def finish_window(self):
        ...
        print(self.fstab_field.text(), self.ip_field.text())       
