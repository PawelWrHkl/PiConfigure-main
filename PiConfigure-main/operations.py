import os
import subprocess
from zabbix_configuration import conf_file
from programs import programs
import tkinter 
#'sudo rpi-imager --cli <.img-path> <device-path>'
...
static domain_name_servers=192.168.101.251'''
    ...
    def get_programs(self,python):
        self.programs_base_path =os.path.join(self.root_path, 'home/pi/Desktop/programy')
        return [names for names,paths in programs[python]]
    ...
    def change_hostname(self,ip):
        self.hostname = f'rpi{ip.split(".")[-1]}'
        ...
        with open (hosts_path, 'r') as file:
            ...
            for line in file :
                if '127.0.1.1' in line and 'rpi' in line:
                    new_hosts_content.append(f'127.0.1.1       {self.hostname}\n')
                else:
    ...
    def zabbix_configure(self):
        full_zabbix_conf_path = os.path.join(self.root_path,'etc/zabbix/zabbix_agent2.conf' )
        new_file = []
        
        for line in conf_file.splitlines():
            if line.startswith('Hostname='):
                line = f'Hostname={self.hostname}'
        ...
        with open(full_zabbix_conf_path,'w')as file:
            file.write(new_file)
    
    
    def set_camera(self):
        cameras ={
            'Arducam 64':'dtoverlay=arducam-64mp',
            "Aparat HQ": 'dtoverlay=imx477',
        }
